...
export const explodex = (x, y, uu) => {
  // let ratio = window.devicePixelRatio;
  console.log(x, y);
  let c = top.document.createElement(`div`);
  let style = uu.getExplosionDecoration();
  c.style = objectToCssString(style);
  c.style.position = "absolute";
  c.style.left = "calc("+x+ "px "+style.left+")";
  c.style.top = "calc("+y + "px "+style.top+")";
  c.style.pointerEvents = "none";
  c.style.zIndex = 1000;
  top.document.body.appendChild(c);
  let time = 1000;
  if (uu.getConfigValue("explosions.duration") !== 0) {
    time = uu.getConfigValue("explosions.duration");
  }
  setTimeout(() => {
